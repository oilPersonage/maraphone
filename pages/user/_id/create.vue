<template>
  <div class="catalogBigContainer">
    <div class="title">Создание курса</div>
    <div class="createBox">
      <button @click="saveCourse" class="saveButton absolute">
      ...
      <div class="createCourseBox">
        ...
        <div class="previewCreateItem">
          ...
          <CatalogItem :item="course" :index="0" :create="true" :clearImg="clearImg" :changeImg="changeImg"/>
        </div>
      </div>
      <button @click="onAdd" class="saveButton">
        <font-awesome-icon style="font-size: 18px" :icon="['fas', 'plus']"/>
        <span>Добавить</span>
      </button>
      <transition-group name="list" tag="div">
        <CreateItem :onDelete="onDelete" :onChange="onChange" v-for="item in list" :key="item.id" :item="item"/>
      </transition-group>
    </div>
  </div>
</template>
